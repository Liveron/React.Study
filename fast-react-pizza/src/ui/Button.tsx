...
type Props = PropsWithChildren & {
  disabled?: boolean;
  to?: string;
  type?: "primary" | "base" | "small" | "round" | "secondary";
  onClick?: (e: MouseEvent) => void;
};

function Button({ children, disabled, to, type = "base", onClick }: Props) {
  const base =
  ...
  const styles: { [key: string]: string } = {
    ...
    secondary:
      " text-sm px-4 py-2.5 md:px-6 md:py-3.5 inline-block rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:text-stone-800 focus:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 active:bg-slate-400 disabled:cursor-not-allowed",
  };
